...
export class AnalyticsService {
  constructor(
    ...
    private readonly projectServiceRepo: Repository<ProjectService>,
  ...
  async getTopVendors(days = 30, limit = 3) {
    ...
    for (const country of countries) {
      ...
      const documentsCount = projectIds.length
        ? await this.researchModel.countDocuments({
  ...
  async getDashboard() {
    ...
    const [projectsCreatedToday, matchesCreatedToday, documentsUploadedToday] =
      await Promise.all([
        this.projectRepo
          .createQueryBuilder("p")
    ...
    const topCountries = topCountriesRaw.map((r) => {
      ...
      const percentage = totalProjects
        ? +(100 * (projectCount / totalProjects)).toFixed(1)
        : 0;
      return {
        country: r.country,
        projectCount,
